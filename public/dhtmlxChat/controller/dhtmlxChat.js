...
var dhtmlxChat = {
	...
	,
	application_path : "http://"+window.location.host+"/dhtmlxChat/"
	
	,
	dhtmlx_codebase_path : "http://"+window.location.host+"/codebase4.0/"
	
	,
	icons_path : "http://"+window.location.host+"/dhtmlxChat/icons/"
	...
	start: function (configuration) {
		...
		self._prompt_login( self.uid, function( user_hash )
		{
			if (self.configuration[self.uid].container) {
				self.model.conf_layout.parent = self.configuration[self.uid].container;
			}
			else {
	...
	_window_manager: function () {
		...
		//self.window_manager.setImagePath(self.model.conf_window.image_path); // deprecated on dhtmlx4.0
	}
	...
	_window: function (uid) {
		...
		self.window[uid] = self.window_manager.createWindow("window_dhtmlxChat_" + uid, self.model.conf_window.left + 10,
	...
	_window_login: function (uid) {
		...
		self.window_login[uid] = self.window_manager.createWindow("window_login_dhtmlxChat_" + uid, self.model.conf_window_login.left + 10,
			self.model.conf_window_login.top + 10, self.model.conf_window_login.width, self.model.conf_window_login.height);
		self.window_login[uid].setText("Login");
		
		//self.window_login[uid].setIcon(self.model.conf_window_login.icon, self.model.conf_window_login.icon_dis);  // deprecated on dhtmlx4.0
	...
	_layout: function (uid) {
		...
		if (self.configuration[uid].container) {		
			self.layout[uid] = new dhtmlXLayoutObject( self.model.conf_layout );
			self.status_bar[uid] = self.layout[uid].attachStatusBar();
			self.status_bar[uid].setText(self.str_main_status);
		}
		else {
			self.layout[uid] = self.window[uid].attachLayout( self.model.conf_layout );
		}
	...
	,_tab: function (uid) {
		...
		//self.tab[uid].setImagePath(self.model.conf_tabbar.image_path); // self.application_path // deprecated on dhtmlx4.0
	...
	_dataView_public_messages: function (uid) {
		var self = this;
		self.dataView_public_messages[uid] = self.tab[uid].tabs("start_tab").attachDataView(self.model.conf_dataView_public_messages.settings);
		self.dataView_public_messages[uid].parse(self.model.conf_dataView_public_messages.data, "json");
		...
		self.status_bar_dataView_public_messages[uid].setText("<div class='red_warning'> <img src ='" + self.model.conf_window.image_path + "warning4.png'>  </div>");
	}
	...
	_dataView_online_users: function (uid) {
		var self = this;
		self.dataView_online_users[uid] = self.layout[uid].cells("b").attachDataView(self.model.conf_dataView_online_users.settings);
		self.dataView_online_users[uid].parse(self.model.conf_dataView_online_users.data, "json");
		...
		self.status_bar_dataView_online_users[uid].setText("<div class='red_warning'> <img src ='" + self.model.conf_window.image_path + "warning4.png'> </div>");
	}
	...
	_form: function (uid) {
		...
		self.form[ uid ] = self.layout[uid].cells("c").attachForm(self.model.conf_form.template);
	...
	_form_login: function (uid, callBackSuccessLogin) {
		...
		self.form_login[ uid ] = self.window_login[uid].attachForm(self.model.conf_form_login.template);
		//self.form[ uid ].setSkin(skin);
	...
	_form_message: function (uid) {
		...
		self.form_message[ uid ] = self.layout2[uid].cells("b").attachForm(self.model.conf_form_message.template);
		//self.form[ uid ].setSkin(skin);
	...
	_form_user_properties: function (uid, callBackSuccessLogin) {
		...
		self.form_user_properties[ uid ] = self.layout[uid].cells("c").attachForm(self.model.conf_form_user_properties.template);
		//self.form[ uid ].setSkin(skin);
